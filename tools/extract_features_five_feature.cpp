...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  size_t num_features = blob_names.size();
    ...
    std::string save_file1 = dataset_names[1]+".txt";
    ofstream out1(save_file1.c_str());
    std::string save_file2 = dataset_names[2]+".txt";
    ofstream out2(save_file2.c_str());
  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
	 ...
	 if (i==0)
         ...
         if (i==1)
                out1<<feature_blob_data[d]<<" ";
         if (i==2)
                out2<<feature_blob_data[d]<<" ";
        }
	if (i==0)
		out<<"\n";
        if (i==1)
