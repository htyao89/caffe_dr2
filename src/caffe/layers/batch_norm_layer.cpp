...
void BatchNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->cpu_data();  // input data N*C*K*K
  Dtype* top_data = top[0]->mutable_cpu_data(); // output data N*C*K*K
  int num = bottom[0]->shape(0); // batch_size  N
  int spatial_dim = bottom[0]->count()/(bottom[0]->shape(0)*channels_); //K*K 
